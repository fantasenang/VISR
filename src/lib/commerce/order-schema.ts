...
export const customerSchema = z.object({
  ...
  whatsapp: z
    .string()
    .trim()
    .regex(/^62\d{8,13}$/, "WhatsApp must use normalized Indonesian format"),
  ...
  .superRefine(({ items }, context) => {
    ...
    for (const item of items) {
      ...
      if (!limit || item.quantity > limit) {
        context.addIssue({
          code: "custom",
          message: `Quantity limit exceeded for ${item.sku}`,
          path: ["items"],
        });
      }

      if (item.sku.startsWith("VISR-HALO-")) {
        haloCount += item.quantity;
      }
    }
    ...
    if (haloCount > products.halo.maxCombinedPerOrder) {
      context.addIssue({
        code: "custom",
        message: "Maximum six Halo units per order",
        path: ["items"],
      });
    }
  });
