...
function resolveRateLimitPolicy(pathname: string, method: string): RateLimitPolicy | null {
  ...
  if (pathname.includes("reservation") || pathname.includes("reserve")) {
    return { name: "reservation", limit: 10 };
  }
  if (pathname.includes("payment") || pathname.includes("midtrans")) {
    return { name: "payment", limit: 10 };
  }

  if (method === "POST" || method === "PUT" || method === "PATCH" || method === "DELETE") {
    return { name: "api-write", limit: 30 };
  }
...
export function proxy(request: NextRequest) {
  ...
  if (policy) {
    ...
    const bucketKey = `${policy.name}:${clientAddress}`;
