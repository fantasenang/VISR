...
export async function POST(request: Request) {
  ...
  try {
    ...
    const liveRate = liveRates.find(
      (rate) => rate.courierCode === selectedCourier && rate.service === parsed.data.shipping.service,
    );

    if (!liveRate) {
      return NextResponse.json({ error: "SHIPPING_SERVICE_UNAVAILABLE" }, { status: 409 });
    }

    if (liveRate.costIdr !== parsed.data.shipping.quotedCostIdr) {
      return NextResponse.json(
        { error: "SHIPPING_RATE_CHANGED", currentCostIdr: liveRate.costIdr },
        { status: 409 },
      );
    }
    ...
    const orderResponse = await fetch(
      `${supabaseUrl}/rest/v1/orders?id=eq.${reservation.order_id}&select=subtotal_idr`,
    ...
    const [orderUpdateResponse, shipmentResponse] = await Promise.all([
      fetch(`${supabaseUrl}/rest/v1/orders?id=eq.${reservation.order_id}`, {
        method: "PATCH",
        headers: { ...headers, Prefer: "return=minimal" },
        body: JSON.stringify({ shipping_cost_idr: shippingCostIdr, total_idr: totalIdr }),
        cache: "no-store",
      }),
    ...
    if (!orderUpdateResponse.ok || !shipmentResponse.ok) {
