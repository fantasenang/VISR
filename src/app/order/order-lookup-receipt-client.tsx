...
export function OrderLookupReceiptClient() {
  ...
  const [paymentReturn, setPaymentReturn] = useState<"finish" | "pending" | null>(null);
  ...
  useEffect(() => {
    ...
    if (returnedPayment === "finish" || returnedPayment === "pending") {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          apiMessage(
            payload,
            "We could not match that order number and contact detail.",
          ),
        );
      }
      ...
      window.history.replaceState(
        {},
        "",
        `/order?order_number=${encodeURIComponent(payload.order.orderNumber)}`,
      );
    } catch (lookupError) {
      setOrder(null);
      setError(
  ...
  const downloadReceipt = async () => {
    if (!order || order.paymentStatus !== "paid" || isDownloadingReceipt) return;
    ...
    try {
      const response = await fetch("/api/orders/receipt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderNumber: order.orderNumber, contact }),
      });
      ...
      const filename =
        ...
        `VISR-Receipt-${order.orderNumber}.pdf`;
      ...
      <div className="visr-container py-12 md:py-20">
        <div className="flex items-center justify-between gap-4">
          ...
          {order && (
            <button
              type="button"
              onClick={resetLookup}
              className="visr-label text-white/45 transition hover:text-white"
            >
              View another order
            </button>
          )}
        ...
        {!order ? (
          <div className="mx-auto max-w-3xl py-20 md:py-28">
            {paymentReturn && (
              <div className="mb-10 rounded-[2rem] border border-white/15 bg-white/[0.055] p-6 md:p-8">
                <p className="visr-label text-white/42">
                  {paymentReturn === "finish" ? "Payment received" : "Payment pending"}
                </p>
                <p className="mt-4 text-xl leading-8">
                  {paymentReturn === "finish"
                    ? "Verify your order below. The PDF receipt appears as soon as payment is confirmed."
                    : "Use your order details below to follow payment verification."}
                </p>
              </div>
            )}
            ...
            <form onSubmit={lookupOrder} className="mt-12 rounded-[2rem] border border-white/12 bg-white/[0.035] p-7 md:p-10">
              <label className="block text-sm text-white/65">
                ...
                <input
                  ...
                  placeholder="VISR.B02.20260807.001"
              ...
              {error && (
                <div className="mt-6 rounded-2xl border border-red-400/30 bg-red-400/10 p-5 text-sm leading-6 text-red-200">{error}</div>
              )}
          ...
          <div className="py-16 md:py-24">
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <button
                ...
                onClick={() => lookupOrder()}
                disabled={isLoading}
                className="rounded-full border border-white/15 px-5 py-3 text-sm transition hover:bg-white hover:text-black disabled:opacity-45"
              >
              ...
              {order.paymentStatus === "paid" && (
                <button
                  ...
                  className="rounded-full bg-white px-5 py-3 text-sm font-medium text-black transition hover:bg-white/85 disabled:opacity-55"
                >
                  {isDownloadingReceipt ? "Preparing PDF…" : "Download Payment Receipt"}
                </button>
              )}
            </div>

            {receiptError && (
              <div className="mt-6 max-w-2xl rounded-2xl border border-red-400/30 bg-red-400/10 p-5 text-sm leading-6 text-red-200">{receiptError}</div>
            )}
            ...
            <div className="mt-14 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
                ...
                <div className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-7">
                  <div>
                    ...
                    {order.paymentStatus === "paid" && order.paidAt && (
                      <p className="mt-3 text-xs leading-5 text-white/40">Payment verified {dateTime(order.paidAt)} WIB</p>
                    )}
                  </div>
              ...
              <aside className="space-y-6">
                {order.paymentStatus === "paid" && (
                  <div className="rounded-[2rem] border border-white/18 bg-white/[0.06] p-7 md:p-9">
                    <p className="visr-label text-white/40">Payment receipt</p>
                    <p className="mt-5 text-2xl">Your PDF receipt is ready.</p>
                    <p className="mt-3 text-sm leading-6 text-white/48">Includes the paid items, shipping charged, total payment, customer details, and verification date.</p>
                    <button
                      type="button"
                      onClick={downloadReceipt}
                      disabled={isDownloadingReceipt}
                      className="mt-7 w-full rounded-full bg-white px-5 py-4 text-sm font-medium text-black transition hover:bg-white/85 disabled:opacity-55"
                    >
                ...
                <div className="rounded-[2rem] border border-white/12 bg-white/[0.035] p-7 md:p-9">
                  ...
                  <div className="mt-7 border-t border-white/10 pt-6 text-sm">
                    ...
                    <div className="mt-3 flex justify-between gap-4 text-white/50"><span>Shipping paid</span><span>{order.shippingCostIdr > 0 ? formatRupiah(order.shippingCostIdr) : "Free"}</span></div>
                    <div className="mt-5 flex justify-between gap-4 text-lg"><span>Total paid</span><span>{formatRupiah(order.totalIdr)}</span></div>
                  </div>
                </div>
